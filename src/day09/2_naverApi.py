...
# [CODE 2]지정한 URL의 요청을 실행하고 응답을 받는 함수
...
# [CODE 3] 매개변수로 검색대상 , 검색어 , 시작번호 , 한번에표기할개수 를 받아서
...
def getNaverSearch(node, srcText, page_start, display):
    ...
    response_decode = getRequestUrl(url)  # 5. url 요청을 하고 응답 객체 받기 , [code2]
    if response_decode is None:
        return None  # 6. 만약에 url 응답 객체가 없으면 None 반환
    else:
        return json.loads(response_decode)  # 7. 응답객체가 있으면 JSON 형식으로 변환
...
def main():
    ...
    src_text = input('검색어 입력하세요:')  # 2. 사용자 입력으로 받은 검색어 변수
    cnt = 0  # 3. 검색 결과 개수
    json_result = []  # 4. 검색 결과를 정리하여 저장할 리스트 변수

    # 5. 1부터 100까지의 검색 결과를 처리한다. # [code2]네이버 뉴스 검색 결과에 대한 응답을 저장하는 객체
    json_response = getNaverSearch(node, src_text, 1, 100)  # 5. [code 3]
    # json_response{ total:총 검색 결과 개수 , start:검색 시작 위치 , display:한 번에 표시할 검색 결과 개수 , item : 개별 검색 결과  }
    ...
    total = json_response['total']  # 6. 전체 검색 결과 개수
    ...
    while (json_response is not None) and (json_response['display'] != 0):
        ...
        for post in json_response['items']:  # 응답받은 검색 결과 중에서 한 개를 저장한객체
            cnt += 1  # 응답 개수 변수 1증가
        ...
        json_response = getNaverSearch(node, src_text, start, 100)
    #
    ...
    with open(f'{src_text}-naver-{node}.json', 'w', encoding='utf-8') as file:
        json_file = json.dumps(json_result, indent=4, sort_keys=True, ensure_ascii=False)
        file.write(json_file)  # 파일 쓰기
